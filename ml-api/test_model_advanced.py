...
# Test data for advanced model (include engineered features if you want to override defaults)
test_cases = [
    {
        "name": "Typical Bid",
        "features": {
    ...
    },
    {
        ...
        "features": {
            ...
            "overhead_cost": 10000,
            "timeline_days": -5,  # Negative timeline to trigger high_risk
            "profit_margin": 8,   # Low margin
            "roi": 10,
            "contingency": 20,
            "risk_count": 5,
            "permit_count": 3
    ...
    {
        "name": "Efficient, High Margin Bid",
        "features": {
            "bid_total": 60000,
...
def test_advanced_model():
    ...
    print("Testing Advanced ML Model...")
    print("=" * 50)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "error" in result:
                    ...
                    print(f"✅ Success!")
                    print(f"   Prediction: {result['prediction']} ({'Won' if result['prediction'] == 1 else 'Lost'})")
                    print(f"   Probability: {result['probability']:.2%}")
            else:
                print(f"❌ Error: {response.status_code}")
                print(f"   Response: {response.text}")
        except requests.exceptions.ConnectionError:
            print("❌ Connection Error: Make sure the advanced ML API server is running on port 8000")
            break
